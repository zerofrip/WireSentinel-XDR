...
impl<E: XdrEventEmitter> XdrSecurityPolicyEngine<E> {
    ...
    pub fn validate_detection_rule(
        ...
        let cond_count = rule.conditions.as_array().map(|a| a.len() as u32).unwrap_or(1);
        if cond_count > policy.max_detection_rule_conditions {
            self.violation("detection_rule", "too many conditions", &rule.name);
            return Err(XdrError::Security("detection rule exceeds condition limit".into()));
        }
        if rule.conditions.to_string().contains("'; DROP") {
            self.violation("detection_rule", "dangerous pattern", &rule.name);
            return Err(XdrError::Security("dangerous detection rule pattern".into()));
        }
        Ok(())
    }

    pub fn validate_playbook(&self, policy: &XdrSecurityPolicy, playbook: &Playbook) -> XdrResult<()> {
        if !policy.allowed_playbook_kinds.contains(&playbook.playbook_kind) {
    ...
    pub fn validate_response(
        ...
        if !policy.allowed_response_actions.contains(&request.action_kind) {
    ...
    pub fn validate_hunt_query(&self, policy: &XdrSecurityPolicy, hunt: &Hunt) -> XdrResult<()> {
        ...
        if dangerous.iter().any(|p| hunt.query.to_uppercase().contains(&p.to_uppercase())) {
    ...
    fn violation(&self, violation_type: &str, detail: &str, resource: &str) {
        ...
        self.emitter.emit(ServiceEvent::now(ServiceEventInner::XdrSecurityViolation {
            violation_type: violation_type.to_string(),
